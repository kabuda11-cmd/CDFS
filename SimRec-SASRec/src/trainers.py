...
class Trainer:
    ...
    def iteration(self, epoch, dataloader, full_sort=True, train=True):
        ...
        if train:
            ...
            for i, batch in rec_cf_data_iter:
                # batch = [torch.cat([batch[i][j] for i in range(len(batch))], dim=0) for j in range(len(batch[0]))]
                batch = tuple(t.to(self.device) for t in batch)
                _, input_ids, target_pos, target_neg, _ = batch
                #print(input_ids.shape)
                sequence_output = self.model.transformer_encoder(input_ids,flag=False)
                # loss = self.ce_loss(sequence_output, target_pos) + self.model.loss
    ...
    def triplet_margin_loss(self, seq_out, pos_ids, neg_ids, margin=1.0):
        # 获取正样本和负样本的嵌入
        pos_emb = self.model.item_embeddings(pos_ids)  # 正样本嵌入
        neg_emb = self.model.item_embeddings(neg_ids)  # 负样本嵌入

        # 将嵌入展平
        pos = pos_emb.view(-1, pos_emb.size(2))  # [batch_size * seq_length, hidden_size]
        neg = neg_emb.view(-1, neg_emb.size(2))  # [batch_size * seq_length, hidden_size]
        seq_emb = seq_out.view(-1, self.args.hidden_size)  # [batch_size * seq_length, hidden_size]

        # 计算嵌入之间的欧几里得距离
        d_ap = torch.norm(seq_emb - pos, p=2, dim=1)  # 锚点与正样本的距离
        d_an = torch.norm(seq_emb - neg, p=2, dim=1)  # 锚点与负样本的距离

        # 计算三元组损失
        loss = torch.clamp(d_ap - d_an + margin, min=0.0)  # max(d_ap - d_an + margin, 0)

        # 掩码，屏蔽无效位置
        istarget = (pos_ids > 0).view(pos_ids.size(0) * self.model.args.max_seq_length).float()
        loss = torch.sum(loss * istarget) / torch.sum(istarget)  # 只计算有效位置的损失
