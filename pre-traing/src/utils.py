...
class MLP(nn.Module):
    """
    构造了一个len(sizes)-1层MLP
    Args:
        sizes: List，一个len(sizes)-1层的全连接层，里面的内容表示每一层的维度
    ...
    def __init__(
        ...
        for i in range(len(sizes) - 1):
            ...
            sub_layer = nn.Linear(sizes[i], sizes[i + 1], bias=bias)
...
def load_npy(npy_path):
    data = torch.from_numpy(np.load(npy_path, allow_pickle=True))
    return data.float()