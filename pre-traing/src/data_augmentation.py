...
class InsertEPS(object):
    def __init__(self, json_file, insert_rate=0.4, max_insert_num_per_pos=1):
    ...
    def __call__(self, sequence):
        ...
        for index, item in enumerate(copied_sequence):
            if index in insert_idx:
                similar_items = self.item_similarities.get(str(item), [])
                if similar_items:
                    insert_item = int(random.choice(similar_items))
...
class SubstituteEPS(object):
    def __init__(self, json_file, substitute_rate=0.1):
        if os.path.exists(json_file):
            with open(json_file, 'r') as f:
                self.item_similarities = json.load(f)
        self.substitute_rate = substitute_rate
    ...
    def __call__(self, sequence):
        ...
        for index in substitute_idx:
            similar_items = self.item_similarities.get(str(copied_sequence[index]), [])
            if similar_items:
                copied_sequence[index] = int(random.choice(similar_items))
